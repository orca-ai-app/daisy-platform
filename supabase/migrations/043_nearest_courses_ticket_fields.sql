-- 043_nearest_courses_ticket_fields.sql
-- Pre-launch (NTH-11/NTH-15): public course search must carry the new
-- per-ticket session_label and vat_rate (migration 040) so the booking widget
-- can show session details and "incl. VAT" on search results, matching the
-- /book/:token path. Redefines find_nearest_courses (same body as 039 plus
-- the two ticket JSON fields; drop + recreate as the RETURNS TABLE is
-- unchanged but the SQL body differs).
--
-- This is migration 043 — do NOT renumber.
