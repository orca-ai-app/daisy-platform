-- 048_round2_public.sql
-- Round 2 franchisee feedback (Feola, Hannah, 2026-08) — public booking surface.
--
-- G4: show SOLD-OUT classes instead of hiding them. find_nearest_courses has
-- filtered `ci.spots_remaining > 0` since it was first written, so a full class
-- silently disappeared from the search. Feola runs waiting lists and a thin
-- schedule reads worse than a full one, so full classes must still come back,
-- clearly marked "Sold out" with booking disabled. Date/distance ordering is
-- unchanged: a sold-out class sits exactly where its date puts it.
--
-- G1 support: carry da_course_instances.description_override (migration 045)
-- through the search so the widget can prefer the franchisee's own wording over
-- the template description, matching the booking_token path.
--
-- Same body as 043 with those two changes only. RETURNS TABLE gains a column,
-- so drop + recreate is required.
--
-- This is migration 048 — do NOT renumber.
