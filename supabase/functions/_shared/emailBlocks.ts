// Block-based email renderer for HQ-editable templates (da_email_templates.blocks).
// Each template is an ordered array of typed blocks; renderBlocks() produces
// email-safe table HTML in the Daisy shell plus a plain-text alternative.
//
// Paragraph, heading, list and button text support:
//   - a markdown subset: **bold**, *italic*, [label](url)
//   - {{merge}} fields from RenderContext (same set as templates.ts)
//
// KEEP IN SYNC: src/features/hq/emails/renderBlocks.ts in the frontend is a
// byte-identical copy of this module (minus this header) so the editor's live
// preview matches what the send-emails function actually sends. If you change
// one, change both.
